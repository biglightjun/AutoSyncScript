...
@client.on(events.NewMessage(chats=[-1001197524983,-1001530312757,-1001256752435,-1001174443659,-1001284907085,-1001320212725,-1001479368440, my_chat_id], pattern=r'.*店|.*dou.*|.*豆.*|.*gua.*'))
...
@client.on(events.NewMessage(chats=[-1001530312757, my_chat_id]))
async def zoo_shopbean(event):
    """
    东哥好朋友关注店铺
    关注频道：https://t.me/dongdongjd
    """
    cookies = myck(_ConfigFile)
    message = event.message.text
    url = re.findall(re.compile(r"[(](https://api\.m\.jd\.com.*?)[)]", re.S), message)
...
@client.on(events.NewMessage(chats=[-1001169232926,-1001530312757,-1001112847619,-1001284907085,-1001320212725,-1001479368440, my_chat_id], pattern=r"^export jd_zdjr\S"))
...
@client.on(events.NewMessage(chats=[-1001169232926,-1001530312757,-10011128467619,-1001284907085,-1001320212725,-1001320212725,-1001479368440, my_chat_id], pattern=r"^export jd_joinTeam\S"))
async def myexport(event):
    try:
        messages = event.message.text.split("\n")
        end = False
        ...
        if end:
            await jdbot.send_message(chat_id, end)
            await cmd("jtask /jd/scripts/gua_joinTeam.js now") # 组队瓜分京豆团ID更新后自动执行gua_joinTeam.js脚本
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))
...
@client.on(events.NewMessage(chats=[-1001174443659,-1001530312757,-10011128467619,-1001284907085,-1001320212725,-1001320212725,-1001479368440, my_chat_id], pattern=r"^export OPEN_CARD_.*=\".*\"|.*='.*'"))
async def myexport(event):
    try:
        messages = event.message.text.split("\n")
        msg = await jdbot.send_message(chat_id, "监控到新的入会变量，准备自动替换")
        for message in messages:
            kv = message.replace("export ", "").replace("*", "")
            kname = kv.split("=")[0]
            vname = re.findall(r"(\".*\"|'.*')", kv)[0][1:-1]
        ...
        await cmd('jtask /jd/scripts/jd_open_card_by_shopid.js now') #现在的命令是ql的，V4自行改成jtask + 脚本绝对路径 now
    except Exception as e:
        await jdbot.send_message(chat_id, 'something wrong,I\'m sorry\n' + str(e))
        logger.error('something wrong,I\'m sorry\n' + str(e))
